# AI Personal Workspace · Frontend
...
- 机器人使用本地 `public/mascot/workspace-companion.riv`，状态机 `AuthCompanion`，数字输入 `authState`（0–7 对应 idle/login/register/email/password/submitting/success/error；8 为 reduced-motion 静止态）及 `blink`（0/1）。认证与眨眼使用独立状态机图层。
...
- 任务状态、后续待办、项目任务分布复用前端 `demoTasks`。概览内勾选今日任务后，相关指标与摘要同步更新；页面已标注示例数据，状态仅保留在当前概览页面内。
