...
test.describe('workspace business flow', () => {
  ...
  test('create project task and note then see dashboard data', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: /闭环项目/ })).toBeVisible()

    await page.getByRole('link', { name: /闭环项目/ }).click()
    ...
    await expect(page.getByRole('dialog')).toBeHidden()
    await expect(page.getByRole('table').getByText('进行中')).toBeVisible()
