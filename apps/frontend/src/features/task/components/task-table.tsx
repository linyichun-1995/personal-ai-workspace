import type { TaskTableItem } from '@/features/task/types'
...
const statusMap: Record<TaskTableItem['status'], { label: string, tone: 'neutral' | 'info' | 'warning' | 'success' }> = {
...
const priorityMap: Record<TaskTableItem['priority'], { label: string, tone: 'success' | 'warning' | 'danger' }> = {
  low: { label: '低', tone: 'success' },
  medium: { label: '中', tone: 'warning' },
  high: { label: '高', tone: 'danger' },
}

function createTaskColumns(onToggleComplete?: (item: TaskTableItem, completed: boolean) => void, onRowClick?: (item: TaskTableItem) => void) {
      ...
      <div className="flex min-w-52 items-start gap-2">
        {onToggleComplete
          ? (
              <Checkbox
                className="mt-0.5"
                aria-label={`完成任务：${getValue()}`}
                checked={row.original.status === 'done'}
                onClick={event => event.stopPropagation()}
                onChange={(event) => {
                  event.stopPropagation()
                  onToggleComplete(row.original, event.target.checked)
                }}
              />
            )
          : null}
...
export function TaskTable({
  ...
  return (
    <DataTable
      columns={createTaskColumns(onToggleComplete, onRowClick)}
      data={data}
      getRowId={row => row.id}
