...
[文档索引](../README.md) · [目录与扩展约定](../architecture/repository-layout.md)

本文为 V0.1 规划，功能清单不代表全部已实现。当前目录结构以目录与扩展约定为准，已实现接口与启动方式见[后端开发指南](../development/backend.md)和[前端开发指南](../development/frontend.md)。
...
```text
apps/backend/src/main/java/com/example/workspace
├── auth
├── user
├── workspace
├── project
